              ...
              <table class="table">
                <tbody>
                  @foreach ($viewData['artworks'] as $artwork)
                    <tr>
                      <td width="120">
                        <img src="{{ $artwork->getImage() }}" class="img-fluid" alt="{{ $artwork->getTitle() }}">
                      </td>
                      <td>
                        <h5>
                          <a href="{{ route('artwork.show', ['id' => $artwork->getId()]) }}" class="text-decoration-none">
                            {{ $artwork->getTitle() }}
                          </a>
                        </h5>
                        <p class="text-muted">{{ $artwork->getDetails() }}</p>
                        <div class="mt-2">
                          <form method="POST" action="{{ route('cart.remove', ['id' => $artwork->getId()]) }}"
                            class="d-inline">
                            @csrf
                            <input type="hidden" name="id" value="{{ $artwork->getId() }}">
                            <button type="submit" class="btn btn-danger btn-sm">{{ __('cart.remove_item') }}</button>
                          </form>
                        </div>
                      </td>
                      <td>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="d-grid gap-2">
              ...
              <a href="{{ route('artwork.index') }}" class="btn btn-outline-secondary w-100">{{ __('cart.cancel') }}</a>
            </div>
          </div>
        </div>
      </div>

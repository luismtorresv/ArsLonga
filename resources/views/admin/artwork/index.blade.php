    ...
    <div class="container py-4">
        ...
        <div class="table-responsive">
            <table class="table-dark table-bordered table-hover mb-0 table align-middle">
                ...
                <tbody>
                    @forelse ($viewData['artworks'] as $artwork)
                        <tr>
                            <td>{{ $artwork->getId() }}</td>
                            <td>
                                <img src="{{ asset('/storage/' . $artwork->getImage()) }}" alt="Thumbnail"
                                    class="img-thumbnail" style="max-width: 64px; max-height: 64px;">
                            </td>
                            <td>{{ $artwork->getTitle() }}</td>
                            <td>{{ $artwork->getAuthor() }}</td>
                            <td>{{ number_format($artwork->getPrice()) }}</td>
                            <td class="text-center">
                                <a href="{{ route('admin.artwork.show', ['id' => $artwork->getId()]) }}"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        ...
                                        <path
                                            d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM8 10a2 2 0 1 1 0-4 2 2 0 0 1 0 4z" />
                                    </svg>
                                </a>
                                <a href="{{ route('admin.artwork.edit', ['id' => $artwork->getId()]) }}"
                                    class="btn btn-sm btn-outline-warning me-1" title="{{ __('admin.edit') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
                                        <path
                                            d="M12.146.854a.5.5 0 0 1 .708 0l2.292 2.292a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 12.5 5.793 10.207 3.5l1-1zm1.586 2.793-1-1L3 13.086V14h.914l8.879-8.879z" />
                                    </svg>
                                </a>
                                <form method="POST"
                                    action="{{ route('admin.artwork.delete', ['id' => $artwork->getId()]) }}"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            ...
                                            <path
                                                d="M5.5 5.5A.5.5 0 0 1 6 5h4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-.5.5H6a.5.5 0 0 1-.5-.5v-7zM4.118 4.5A1.5 1.5 0 0 1 5.5 3h5a1.5 1.5 0 0 1 1.382 1.5H15a.5.5 0 0 1 0 1h-1v8a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5.5H1a.5.5 0 0 1 0-1h1.118zM2.5 5v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V5h-10z" />
                                        </svg>
                                    </button>
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center">{{ __('admin.noArtworks') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
